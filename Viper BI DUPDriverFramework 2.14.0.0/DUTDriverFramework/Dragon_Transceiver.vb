...
Public Class Dragon_Transceiver
    ...
    Private _CustPort As Integer = 8081
    Private _Model As String = "OTRX_Dragon"
    ...
    Public Structure PSStatusData
        'WILDCAT_P1>ps read 0 8
        'Chan    Description     Counts     Value  Units
        '   0  Csense AISG 28V       5       0.00  A
        '   1  Csense Buck 28V    1437       2.19  A
        '   2  Temperature PSB    1407       35.9  Deg C
        '   3  Vsense      28V    3492      28.02  V
        '   4  Vsense    Vtrim    2037       1.24  V
        '   5  Vsense       5V    3866       5.22  V
        '   6  Vsense     3.3V    3741       3.43  V
        '   7  Vsense AISG 28V       1       0.01  V

        Dim Csense_AISG_28V As Double
        Dim Csense_Buck_28V As Double
        Dim Temperature_PSB As Double
        Dim Vsense_28V As Double
        Dim Vsense_Vtrim As Double
        Dim Vsense_5V As Double
        Dim Vsense_3V3 As Double
        Dim Vsense_AISG_28V As Double

        Dim Csense_AISG_28V_Tick As Integer
        Dim Csense_Buck_28V_Tick As Integer
        Dim Temperature_PSB_Tick As Integer
        Dim Vsense_28V_Tick As Integer
        Dim Vsense_Vtrim_Tick As Integer
        Dim Vsense_5V_Tick As Integer
        Dim Vsense_3V3_Tick As Integer
        Dim Vsense_AISG_28V_Tick As Integer
    End Structure
    ...
    Public Sub Open() Implements IDeviceUnderTest.Open
        Try
            ...
            '_TelnetSession.SendCommandAndReceive("gain temp_slope 0.029")
            _TelnetSession.SendCommandAndReceive("clitimeout disable")
    ...
    Public Sub RefreshFileSystem()
        Try
            Dim Answer As String = _TelnetSession.SendCommandAndReceive("fs dir")
            Dim tmpFile As TRDUFile
            'Dim tmpArr() As String = Answer.Split("|")
            'For index As Integer = 0 To tmpArr.Length - 1
            '    If InStr() Then
            'Next
    ...
    Public Sub RemoveFileFromFileSystem(ByVal FileName As String)
        ...
        Try
            'wenleiming updated to delete log files and FlightRecorder before ship out.
            Dim Answer As String = _TelnetSession.SendCommandAndReceive("fs dir")
            Dim tmpArr() As String = Answer.Split("|")
            For index As Integer = 0 To tmpArr.Length - 1
                If InStr(tmpArr(index), FileName) > 0 Then
                    Dim tmpIndex = CInt(tmpArr(index - 4).Trim)
                    _TelnetSession.TimeOutSec = 120
                    Answer = _TelnetSession.SendCommandAndReceive(String.Format("fs del {0}", tmpIndex))
    ...
    Public Function BootmodeStatus() As BootmodeData
        ...
        For i As Integer = 0 To 1
            ...
            If bd.Image(i).Name = "InvalidImage" Then
                bd.Image(i).Dsp = String.Empty
                bd.Image(i).DspNum = 0

                bd.Image(i).Fpga = String.Empty
                bd.Image(i).FpgaNum = 0

                bd.Image(i).Cpri = String.Empty
                bd.Image(i).CpriNum = 0
            Else
    ...
    Public Function LnaVswrReadEeprom() As VswrTxRecord()
        ...
        Dim vswr(fields.Length - 2) As AndrewIntegratedProducts.DUTDriverFramework.Dragon_Transceiver.VswrTxRecord
        For i As Integer = 0 To vswr.Length - 1
    ...
    Public ReadOnly Property InventoryPA(ByVal channel As Integer) As InventoryDataPa
        Get
            ...
            inv.ManufactureCode = GetInventoryPaField(resp, "Manufacture ?[Cc]ode")
    ...
    Public ReadOnly Property PSStatus() As PSStatusData
        ...
        '   7  Vsense AISG 28V       1       0.01  V
        Get
            Dim cmd As String = "ps read 0 8"
            Dim resp As String = _TelnetSession.SendCommandAndReceive(cmd)
            Dim m As MatchCollection = Regex.Matches(resp, " +\d +(.*) +(\d+) +(\S+) +(.*)")
            Dim data As PSStatusData
            If m.Count <> 8 Then
                Throw New Exception("FpgaRead invalid resp: " & resp)
            End If

            data.Csense_AISG_28V = m(0).Groups(3).Value
            data.Csense_Buck_28V = m(1).Groups(3).Value
            data.Temperature_PSB = m(2).Groups(3).Value
            data.Vsense_28V = m(3).Groups(3).Value
            data.Vsense_Vtrim = m(4).Groups(3).Value
            data.Vsense_5V = m(5).Groups(3).Value
            data.Vsense_3V3 = m(6).Groups(3).Value
            data.Vsense_AISG_28V = m(7).Groups(3).Value

            data.Csense_AISG_28V_Tick = m(0).Groups(2).Value
            data.Csense_Buck_28V_Tick = m(1).Groups(2).Value
            data.Temperature_PSB_Tick = m(2).Groups(2).Value
            data.Vsense_28V_Tick = m(3).Groups(2).Value
            data.Vsense_Vtrim_Tick = m(4).Groups(2).Value
            data.Vsense_5V_Tick = m(5).Groups(2).Value
            data.Vsense_3V3_Tick = m(6).Groups(2).Value
            data.Vsense_AISG_28V_Tick = m(7).Groups(2).Value
            Return data
        End Get
    End Property
    ...
    'Public Sub CustomerCarrierCfg(ByVal Index As Integer, ByVal Power As Double, ByVal TxFreqMHz As Double, ByVal RxFreqMHz As Double, ByVal CarrType As CarrTypeEnum)
    '  '[ 
    '  ' MESSAGE:TYPE=SET 
    '  'TRANSACTION: ID = 103
    '  ' CARRIERCFG: INDEX=1 STATE=ENABLE TXCONTAINER=1 TX2CONTAINER=1 RX1CONTAINER=1 RX2CONTAINER=5 GRPSIZEDOWN=4 GRPSIZEUP=4 TXFREQ=2140000 RXFREQ=1745000 SIGTYPE=NONE POWER=430 CARRTYPE=LTE_5
    '  '] 
    '  _TelnetCustomer.SendCommand("SET", 103, _
    '  String.Format("CARRIERCFG: INDEX={0} STATE=ENABLE TXCONTAINER=1 TX2CONTAINER=1 RX1CONTAINER=1 RX2CONTAINER=5 GRPSIZEDOWN=4 GRPSIZEUP=4 TXFREQ={1} RXFREQ={2} SIGTYPE=NONE POWER={3} CARRTYPE={4}", Index, CInt(TxFreqMHz * 1000), CInt(RxFreqMHz * 1000), CInt(Power * 10), CarrType.ToString))
